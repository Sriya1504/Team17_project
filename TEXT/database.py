        ...
        with self.get_connection() as conn:
            ...
            cursor.execute(
                ...
                SELECT 
                    ...
                    AVG(total_marks) as overall_avg,
                    MAX(total_marks) as highest_mark,
                    MIN(total_marks) as lowest_mark,
                    COUNT(CASE WHEN total_marks >= 40 THEN 1 END) * 100.0 / COUNT(*) as pass_percentage
                FROM students_results
                WHERE roll_number = ?
                """,
                (roll_number,)
            ...
            overall_stats = {
                "total_exams": stats_row[0] if stats_row else 0,
                "overall_average": round(stats_row[1], 2) if stats_row and stats_row[1] else 0,
                "highest_mark": stats_row[2] if stats_row else 0,
                "lowest_mark": stats_row[3] if stats_row else 0,
                "pass_percentage": round(stats_row[4], 2) if stats_row and stats_row[4] else 0
            }
